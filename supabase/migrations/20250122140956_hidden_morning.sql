/*
  # Add Sold Products Table

  1. New Tables
    - `sold_products`
      - `sold_product_id` (text, primary key)
      - `receipt_id` (text, not null)
      - `product_name` (text, not null)
      - `quantity` (integer, not null)
      - `unit_price` (numeric, not null)
      - `total_price` (numeric, not null)
      - `vendor_code_if_by_vendor` (text)
      - `business_code` (text, not null)
      - `business_bracnh_name` (text, not null)
      - `comission_for_bussnies_from_vendor` (numeric)
      - `created_at` (timestamp with time zone)

  2. Security
    - Enable RLS on `sold_products` table
    - Add policies for authenticated users
*/

-- Create sold_products table
CREATE TABLE IF NOT EXISTS sold_products (
    sold_product_id TEXT PRIMARY KEY,
    receipt_id TEXT NOT NULL,
    product_name TEXT NOT NULL,
    quantity INTEGER NOT NULL,
    unit_price NUMERIC NOT NULL,
    total_price NUMERIC NOT NULL,
    vendor_code_if_by_vendor TEXT,
    business_code TEXT NOT NULL,
    business_bracnh_name TEXT NOT NULL,
    comission_for_bussnies_from_vendor NUMERIC,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- Create indexes for better query performance
CREATE INDEX IF NOT EXISTS idx_sold_products_receipt_id ON sold_products(receipt_id);
CREATE INDEX IF NOT EXISTS idx_sold_products_business_code ON sold_products(business_code);
CREATE INDEX IF NOT EXISTS idx_sold_products_vendor_code ON sold_products(vendor_code_if_by_vendor);

-- Enable RLS
ALTER TABLE sold_products ENABLE ROW LEVEL SECURITY;

-- Create RLS policies
CREATE POLICY "Users can view their own sold products"
ON sold_products
FOR SELECT
TO authenticated
USING (
    business_code = auth.jwt()->>'business_code' OR
    vendor_code_if_by_vendor = auth.jwt()->>'business_code'
);

CREATE POLICY "Users can insert their own sold products"
ON sold_products
FOR INSERT
TO authenticated
WITH CHECK (
    business_code = auth.jwt()->>'business_code'
);

-- Add function to generate sold product ID
CREATE OR REPLACE FUNCTION generate_sold_product_id()
RETURNS TEXT
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
DECLARE
    new_id TEXT;
BEGIN
    -- Generate a random ID with SPD prefix (Sold Product ID)
    new_id := 'SPD' || UPPER(SUBSTRING(MD5(RANDOM()::TEXT) FROM 1 FOR 8));
    
    -- Keep generating until we find a unique one
    WHILE EXISTS (SELECT 1 FROM sold_products WHERE sold_product_id = new_id) LOOP
        new_id := 'SPD' || UPPER(SUBSTRING(MD5(RANDOM()::TEXT) FROM 1 FOR 8));
    END LOOP;
    
    RETURN new_id;
END;
$$;